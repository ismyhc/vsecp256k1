...
fn test_xonly_pubkey_creation_and_serialization() {
    ...
    x_pubkey, parity := ctx.create_xonly_pubkey(keypair) or {
        assert false, 'Failed to create xonly pubkey: ${err}'
        return
    }
    assert !isnil(x_pubkey), 'Xonly pubkey should not be nil'
    
    // Check parity based on its type
    $if parity is bool {
        assert parity == true || parity == false, 'Parity should be a boolean'
    } $else $if parity is int {
        assert parity == 0 || parity == 1, 'Parity should be 0 or 1'
    } $else {
        assert false, 'Unexpected type for parity'
    }
